...
app.post('/chat/:provider', async (req, res) => {
    try {
        ...
        if (provider === 'openai') {
            const { chatWithOpenAI } = await Promise.resolve().then(() => require('./adapters/openai'));
            response = await chatWithOpenAI(modelConfig, prompt, { maxTokens, temperature, streaming: false, apiKey });
        }
        else if (provider === 'anthropic') {
            const { chatWithAnthropic } = await Promise.resolve().then(() => require('./adapters/anthropic'));
            response = await chatWithAnthropic(modelConfig, prompt, { maxTokens, temperature, streaming: false, apiKey });
        }
        else if (provider === 'gemini') {
            const { chatWithGemini } = await Promise.resolve().then(() => require('./adapters/gemini'));
